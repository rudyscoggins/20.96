...
namespace Examples.Operations
{
  ...
  {
    public RegistrationConfigurations(HttpClient USISDKClient) : base(USISDKClient)
    {
    }
    ...
    {
      return APIUtil.GetRegistrationConfiguration(USISDKClient, orgCode, configurationCode);
    }
    ...
    /// </summary> 
    public IEnumerable<RegistrationConfigurationsModel> GetWithPublicLanguages(string orgCode, string configurationCode)
    {
      SearchMetadataModel searchMetadata = null;
      return APIUtil.GetSearchList<RegistrationConfigurationsModel>(USISDKClient, ref searchMetadata, orgCode, $"ConfigurationCode eq '{configurationCode}'", "", 10000, 10000, new List<string> { "[PublicLanguages]" });
    }
    ...
    {
      SearchMetadataModel searchMetadata = null;
      return APIUtil.GetSearchList<RegistrationConfigurationsModel>(USISDKClient, ref searchMetadata, orgCode, $"ConfigurationCode eq '{configurationCode}'", "", 10000, 10000, new List<string> { "[PublicLanguages],[RegistrationPreferenceTypes]" });
    }

    /// <summary>
    /// A retrieve all.  We recommend using a specific query when searching, shown in the General class.
    /// </summary> 
    public IEnumerable<RegistrationConfigurationsModel> RetrieveAll(string orgCode)
    {
      SearchMetadataModel searchMetadata = null;
      return APIUtil.GetSearchList<RegistrationConfigurationsModel>(USISDKClient, ref searchMetadata, orgCode, "All");
    }

    /// <summary>
    /// A retrieve by odata query.  We recommend using a specific query when searching, shown in the General class.
    /// </summary> 
    public IEnumerable<RegistrationConfigurationsModel> RetrieveByOData(string orgCode, string oData)
    {
      SearchMetadataModel searchMetadata = null;
      return APIUtil.GetSearchList<RegistrationConfigurationsModel>(USISDKClient, ref searchMetadata, orgCode, oData);
    }
    ...
    public RegistrationConfigurationsModel Edit(string orgCode, string configurationCode, string newDescription)
    {
      ...
      return APIUtil.UpdateRegistrationConfiguration(USISDKClient, regConfig);
    }
    ...
    {
      APIUtil.AwaitDeleteRegistrationConfiguration(USISDKClient, orgCode, configurationCode).Wait();  //Only error responses are returned from Delete calls --.Wait() allows errors to catch properly here.
    }
